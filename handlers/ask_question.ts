...
export async function handleAskQuestion(hook: PreToolUseHook): Promise<void> {
  ...
  const elements: ThermalElement[] = [
    ...
    { type: "text", content: "CLAUDE PREGUNTA", align: "center", bold: true },
    { type: "datetime", content: "HH:mm", align: "center" },
    { type: "text", content: `[${project}]`, align: "left" },
    { type: "separator" },
  ];
  ...
  for (const q of questions.slice(0, 3)) {
    elements.push({
      type: "text",
      content: wrapLines(q.question, 32),
      align: "left",
      bold: true,
    });
    ...
    if (q.options && q.options.length > 0) {
      for (const opt of q.options.slice(0, 4)) {
        elements.push({ type: "text", content: `  > ${opt.label}`, align: "left" });
      }
    }
  ...
  elements.push({ type: "line", content: "-", align: "center" });
